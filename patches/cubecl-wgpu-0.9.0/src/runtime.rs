...
pub(crate) fn create_server(setup: WgpuSetup, options: RuntimeOptions) -> WgpuServer {
    ...
    if adapter_limits.min_subgroup_size == 0 && adapter_limits.max_subgroup_size == 0 {
        ...
        // Cap max_subgroup_size so that cubek-reduce's workgroup (plane_size * 8)
        // stays within max_compute_invocations_per_workgroup (256 on WebGPU).
        let max_for_reduce = adapter_limits.max_compute_invocations_per_workgroup / 8;
        adapter_limits.max_subgroup_size = max_for_reduce.min(128);
    }
